...
internal class SelectorRangeBounds<T : Comparable<T>> private constructor(
    ...
    private val lower: SelectorRangeBound<T>?,
    private val upper: SelectorRangeBound<T>?,
) {
    val minimum: T? get() = lower?.value
    val maximum: T? get() = upper?.value
    ...
    companion object {
        fun <T : Comparable<T>> exact(
            ...
            SelectorRangeBound(value, offset).let { bound ->
                SelectorRangeBounds(reader, lower = bound, upper = bound)
            }
        ...
        fun <T : Comparable<T>> ranged(
            ...
            SelectorRangeBounds(
                reader = reader,
                lower = minimum?.let { SelectorRangeBound(it, minimumOffset) },
                upper = maximum?.let { SelectorRangeBound(it, maximumOffset) },
            )
    }
}
...
private data class SelectorRangeBound<T : Comparable<T>>(
    val value: T,
    val offset: Int,
)
