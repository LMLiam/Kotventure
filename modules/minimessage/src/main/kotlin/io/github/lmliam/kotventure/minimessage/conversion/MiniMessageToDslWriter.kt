 ...
 *
 * The walker decides *what* to emit; [KotlinSourceBuilder] owns indentation and the `*Literal` helpers render leaf
 * values, so each function reads as the DSL it produces. Every payload MiniMessage can produce has a DSL form; the few
 * shapes that have no surface (an identity-less or multi-source player head, player-head profile properties, custom
 * click actions, raw data-component values) are rejected at the point of emission rather than dropped silently.
 */
 ...
 * builders (`text`, `translatable`, …) are `ComponentScope` members that return `Unit`, so an unwrapped `text("…")`
 * could not type-check as a `ComponentLike` value. Both `translatable`'s `arg` and `selector`'s `separator` expose a
 * `TextScope` block overload for exactly this.
 */
    ...
    block("click") {
        when (event.action()) {
            ...
            ClickEvent.Action.COPY_TO_CLIPBOARD -> line("copy(\"${escapeKotlinString(event.textPayload())}\")")
                ...
                "miniToDsl cannot represent the ${event.action().name()} click action.",
            )
        }
    }
}
