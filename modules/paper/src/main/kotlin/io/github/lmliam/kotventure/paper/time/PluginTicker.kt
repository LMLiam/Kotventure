 ...
 * `500.milliseconds` is valid and `75.milliseconds` is not valid. [Ticker.once] has the same rule
 * for a positive delay. A delay of zero selects the next tick.
 *
 * [Ticker.ownsCurrentThread] gives the result of `Server.isGlobalTickThread`.
 *
 ...
 * @sample io.github.lmliam.kotventure.paper.time.onceTickerSample
 */
 ...
 *
 * [Ticker.repeating] and [Ticker.once] fail if Paper has already retired the entity scheduler.
 * [Ticker.ownsCurrentThread] gives the result of `Server.isOwnedByCurrentRegion` for the entity.
 *
 ...
 *
 * Each [Ticker.repeating] and [Ticker.once] call reads the current world and chunk from [location].
 ...
 * [Ticker.ownsCurrentThread] gives the result of `Server.isOwnedByCurrentRegion` for the location.
 *
