...
`open(...)` creates an open-file event for `file:` URIs and an open-URL event otherwise; `openUrl(...)` and
`openFile(...)` remain available when you want the action to be explicit. `runCommand(...)`, `suggestCommand(...)`, and
`copyToClipboard(...)` are kept as aliases for `run(...)`, `suggest(...)`, and `copy(...)`.
