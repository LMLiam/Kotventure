...
function isCodeqlCategory(value: string | undefined): value is CodeqlCategory {
  return CODEQL_CATEGORIES.includes(value as CodeqlCategory);
}

export function parseCodeqlArtifactName(name: string): ParsedCodeqlArtifactName | null {
  const categoryPattern = CODEQL_CATEGORIES.join('|');
  const match = name.match(new RegExp(`^${CODEQL_ARTIFACT_PREFIX}(${categoryPattern})-(\\d+)-(\\d+)-(\\d+)-([0-9a-f]{40})$`));
  ...
  if (!isCodeqlCategory(category)) return null;
