...
const EXPECTED_FILE_NAME = Buffer.from(RESULT_FILE_NAME, 'utf8');

function rejectArchive(message) {
    throw new Error(`metrics artifact archive ${message}`);
}

function assertRange(archive, offset, length, label) {
...
function readCentralDirectoryEntry(archive) {
    ...
    if (!fileName.equals(EXPECTED_FILE_NAME)) {
        rejectArchive(`must contain only ${RESULT_FILE_NAME}`);
    }
...
function readCompressedData(archive, entry) {
...
function extractMetricsResultArchive(archive) {
    ...
    const entry = readCentralDirectoryEntry(archive);
    const compressedData = readCompressedData(archive, entry);
