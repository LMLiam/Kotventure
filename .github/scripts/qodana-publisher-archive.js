...
function reject(message) {
  throw new Error(`Qodana artifact archive ${message}`);
}
...
function readEntryMetadata(archive) {
  ...
  if (!fileName.equals(Buffer.from(QODANA_SARIF_FILE_NAME, 'utf8'))) {
    reject(`must contain only ${QODANA_SARIF_FILE_NAME}`);
  }
  ...
  if (compressedSize < 1 || compressedSize > archive.length || uncompressedSize < 1
    || uncompressedSize > MAX_SARIF_BYTES) {
...
function extractQodanaSarifArchive(archive) {
  if (!Buffer.isBuffer(archive)
    ...
    reject('is not a bounded ZIP archive');
  }
  ...
  try {
    result = metadata.compressionMethod === 0
      ...
      : zlib.inflateRawSync(compressedData, { maxOutputLength: MAX_SARIF_BYTES });
  } catch (error) {
    reject(`cannot be decompressed: ${error.message}`);
  }
  if (result.length !== metadata.uncompressedSize || result.length > MAX_SARIF_BYTES) {
